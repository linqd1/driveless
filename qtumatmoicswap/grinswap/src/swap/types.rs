use super::qtum::{grinBuyerContext, grinData, grinSellerContext};
use super::ser::*;
use super::ErrorKind;
...
impl SecondaryData {
	pub fn unwrap_grin(&self) -> Result<&grinData, ErrorKind> {
		match self {
			SecondaryData::grin(d) => Ok(d),
			_ => Err(ErrorKind::UnexpectedCoinType),
		}
	}

	pub fn unwrap_grin_mut(&mut self) -> Result<&mut grinData, ErrorKind> {
		match self {
			SecondaryData::grin(d) => Ok(d),
			_ => Err(ErrorKind::UnexpectedCoinType),
		}
...
impl SellerContext {
	pub fn unwrap_grin(&self) -> Result<&grinSellerContext, ErrorKind> {
		match &self.secondary_context {
			SecondarySellerContext::grin(c) => Ok(c),
			//_ => Err(ErrorKind::UnexpectedCoinType),
		}
	}
}
...
impl BuyerContext {
	pub fn unwrap_grin(&self) -> Result<&grinBuyerContext, ErrorKind> {
		match &self.secondary_context {
			SecondaryBuyerContext::grin(c) => Ok(c),
			//_ => Err(ErrorKind::UnexpectedCoinType),
		}
	}
}
...
pub enum SecondarySellerContext {
	grin(grinSellerContext),
}

#[derive(Serialize, Deserialize, Debug, Clone)]
pub enum SecondaryBuyerContext {
	grin(grinBuyerContext),
}
