...
use qtum::{Address, OutPoint, Transaction};
...
pub trait grinNodeClient: Sync + Send + 'static {
...
#[derive(Debug, Clone)]
pub struct TestgrinNodeClientState {
	pub height: u64,
...
#[derive(Debug, Clone)]
pub struct TestgrinNodeClient {
	pub state: Arc<Mutex<TestgrinNodeClientState>>,
}

impl TestgrinNodeClient {
	pub fn new(height: u64) -> Self {
		Self {
			state: Arc::new(Mutex::new(TestgrinNodeClientState {
				height,
...
impl grinNodeClient for TestgrinNodeClient {
	fn height(&mut self) -> Result<u64, ErrorKind> {
