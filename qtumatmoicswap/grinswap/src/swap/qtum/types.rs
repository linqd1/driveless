...
use qtum::blockdata::opcodes::{all::*, OP_FALSE};
use qtum::blockdata::script::Builder;
use qtum::consensus::Encodable;
use qtum::network::constants::Network as grinNetwork;
use qtum::{Address, Script, Transaction, TxIn, TxOut, VarInt};
use qtum_hashes::sha256d;
...
pub struct grinData {
	...
	pub locked: bool,
	pub redeem_tx: Option<grinRedeemTx>,
...
impl grinData {
	...
	pub(crate) fn from_offer<K>(
		keychain: &K,
		offer: grinOfferUpdate,
		context: &grinBuyerContext,
	) -> Result<Self, ErrorKind>
	where
		K: Keychain,
	{
	...
	pub(crate) fn accepted_offer(
		&mut self,
		accepted_offer: grinAcceptOfferUpdate,
	) -> Result<(), ErrorKind> {
	...
	pub(crate) fn redeem_tx(
		...
		self.redeem_tx = Some(grinRedeemTx {
			txid: tx.txid(),
	...
	fn redeem_script_sig(
		...
		let script_sig = Builder::new()
			.push_opcode(OP_FALSE) // qtum multisig bug
	...
	pub(crate) fn offer_update(&self) -> grinUpdate {
		grinUpdate::Offer(grinOfferUpdate {
			lock_time: self.lock_time,
			cosign: self.cosign.clone(),
		})
	}

	pub(crate) fn accept_offer_update(&self) -> Result<grinUpdate, ErrorKind> {
		Ok(grinUpdate::AcceptOffer(grinAcceptOfferUpdate {
...
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct grinSellerContext {
	pub cosign: Identifier,
}

#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct grinBuyerContext {
...
pub enum grinUpdate {
	Offer(grinOfferUpdate),
	AcceptOffer(grinAcceptOfferUpdate),
}

impl grinUpdate {
	pub fn unwrap_offer(self) -> Result<grinOfferUpdate, ErrorKind> {
		match self {
			grinUpdate::Offer(u) => Ok(u),
			_ => Err(ErrorKind::UnexpectedMessageType),
		}
	}

	pub fn unwrap_accept_offer(self) -> Result<grinAcceptOfferUpdate, ErrorKind> {
		match self {
			grinUpdate::AcceptOffer(u) => Ok(u),
			_ => Err(ErrorKind::UnexpectedMessageType),
		}
	}

	pub fn wrap(self) -> SecondaryUpdate {
		SecondaryUpdate::grin(self)
	}
}

#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct grinOfferUpdate {
	pub lock_time: u32,
	#[serde(serialize_with = "pubkey_to_hex", deserialize_with = "pubkey_from_hex")]
	pub cosign: PublicKey,
}
...
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct grinAcceptOfferUpdate {
	#[serde(serialize_with = "pubkey_to_hex", deserialize_with = "pubkey_from_hex")]
	pub refund: PublicKey,
}

fn grin_network(network: Network) -> grinNetwork {
	match network {
		Network::Floonet => grinNetwork::Testnet,
		Network::Mainnet => grinNetwork::qtum,
	}
}
...
mod tests {
	use super::*;
	use qtum::util::address::Payload;
	use qtum::util::key::PublicKey as grinPublicKey;
	use qtum::OutPoint;
	use qtum_hashes::{hash160, Hash};
