...
import (
	...
	"github.com/DesWurstes/qtumd/txscript"
	"github.com/qtumsuite/qtumd/chaincfg"
	"github.com/qtumsuite/qtumd/chaincfg/chainhash"
	rpc "github.com/qtumsuite/qtumd/rpcclient"
	txscript2 "github.com/qtumsuite/qtumd/txscript"
	"github.com/qtumsuite/qtumd/wire"
	"github.com/qtumsuite/qtumutil"
	"github.com/qtumsuite/qtumwallet/wallet/txrules"
	"github.com/cpacia/grinutil"
...
func init() {
	flagset.Usage = func() {
		fmt.Println("Usage: grinatomicswap [flags] cmd [cmd args]")
		fmt.Println()
...
func createSig(tx *wire.MsgTx, idx int, pkScript []byte, addr qtumutil.Address,
	...
	sig, err = grinutil.RawTxInSignature(tx, idx, pkScript, txscript2.SigHashAll, wif.PrivKey, val)
	if err != nil {
		return nil, nil, err
	}
...
func fundTransactionJsonParser(rawResp json.RawMessage) (fundedTx *wire.MsgTx, fee qtumutil.Amount, err error) {
	var resp struct {
		Hex       string  `json:"hex"`
...
func (cmd *initiateCmd) runCommand(c *rpc.Client) error {
	...
	fmt.Printf("Contract fee: %0.8f grin (%0.8f grin/kB)\n", b.contractFee.Toqtum(), contractFeePerKb)
	fmt.Printf("Refund fee:   %0.8f grin (%0.8f grin/kB)\n\n", b.refundFee.Toqtum(), refundFeePerKb)
...
func (cmd *redeemCmd) runCommand(c *rpc.Client) error {
	...
	if txrules.IsDustOutput(redeemTx.TxOut[0], minFeePerKb) {
		return fmt.Errorf("redeem output value of %0.8f grin is dust", float64(redeemTx.TxOut[0].Value)*1e-8)
	}
	...
	fmt.Printf("Redeem fee: %0.8f grin (%0.8f grin/kB)\n\n", fee.Toqtum(), redeemFeePerKb)
...
func (cmd *refundCmd) runCommand(c *rpc.Client) error {
	...
	fmt.Printf("Refund fee: %0.8f grin (%0.8f grin/kB)\n\n", refundFee.Toqtum(), refundFeePerKb)
	fmt.Printf("Refund transaction (%v):\n", &refundTxHash)
	fmt.Printf("%x\n\n", buf.Bytes())
